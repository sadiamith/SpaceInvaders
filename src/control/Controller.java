...
/** 
 * The class to start the space invaders game.  It is also the 
 * controller in the model-view-controller architecture of the system,
 * i.e., when an event arrives, it takes appropriate action to update 
 * the game and the view.  It also observes when the game is over to
 * respond to it. 
 */
public class Controller implements KeyListener, ActionListener, GameObserver
{
	...
	/** An interface to the game with the methods that can be used to access information
	 * from the game.
	 */
	private GameInfoProvider gameInfo;

	/**
	 * When an action event is received, take the action specified by
	 * its command: start a new game, save the score from the last game,
	 * show the welcome view, or show the high scores.
	...
	public void actionPerformed(ActionEvent event)
	{
		...
		else if (actionCommand.equals("savescore"))
		{
			...
			{
				GameResult result = new GameResult(id, gameInfo.getPlayerScore(), 
				                                       gameInfo.getLevel());
	...
	/**
	 * When notified of a game change, check for the end of the game.
	 * If it is the end of game, show that final view for 2.5 seconds, 
	 * and then either display the welcome view, or, if the score is 
	 * high enough to be saved, display the save score view. 
	 */
	public void gameChanged()
	{
		if (gameInfo.isOver())
		{
			...
			/* Instead of just putting this thread to sleep, a Timer is needed 
			 * as the current thread (the event dispatch thread) needs to continue 
			 * execution to update the view to show the game over message. */
			t.setRepeats(false);
			t.start();
		}
	}
