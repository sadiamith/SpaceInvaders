...
/**
 * A panel across the top that displays the score, the level, 
 * and the number of remaining lives of the user. It is a 
 * GameModelObserver, so method gameChanged() is invoked whenever
...
public class GameInfoPanel extends GraphicsPanel implements GameObserver
{
	 ...
	 * @param gameInfo	the interface to the game used to 
	...
	/**
	 * When the game changes, repaint the panel to show the latest
	 * values.
	 */
	...
	/**
	 * In the upper lefthand corner, give the label Lives, and
	 * draw an icon for each life recorded in lives.  Draw them
	 * in rows with LIVES_PER_ROW icons per row.
	...
	private void drawLives(int lives, Graphics2D graphics)
	{
		...
		{
			drawImage(xOffset + (col * LIVES_WIDTH), yOffset,
			          LIVES_WIDTH, LIVES_HEIGHT, imageName, graphics);
			col++;
