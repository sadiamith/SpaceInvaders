...
/**
 * The view of the model-view controller architecture.
 * This view is a frame used to display various views of the game 
 * and its results. 
 */
public class View extends JFrame
{
	 ...
	 * When the game is being shown, the game and score panels 
	 * observe changes in the game, and use a game provider to access 
	 * information from the game.
	 */

	/**
	 * Initialize the frame for the various views to be inserted into it,
	 * where width and height are the dimensions for the game.
	 */
	public View(int width, int height)
	{
		setTitle("Space Invaders");
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		/* Width and height passed in are those for the game panel, so add 
		 * enough height for the information panel, and pack the frame with
		 * the contentPane size set to its preferred size.  */
		Container pane = getContentPane();
		pane.setLayout(new BorderLayout());
		pane.setPreferredSize(new Dimension(width, height + GameInfoPanel.HEIGHT));
		pack();
		setResizable(false);
	}

	/**
	 * Set the frame visible showing the panel passed in, 
	 * and set the focus in the Component obtained from the panel.
	...
	/**
	 * Construct the view to allow the user to save his/her score, 
	 * and make the view visible.
	 ...
	 */
	public void showSaveScoreView(GameInfoProvider gameInfoProvider,
	                              ActionListener actionListener)
	{
		ViewPanel panel =  new SaveScorePanel(getWidth(), getHeight(), 
		                                      gameInfoProvider.getPlayerScore(), 
		                                      gameInfoProvider.getLevel(), actionListener);
		displayPanel(panel);
	}

	/**
	 * Construct and make visible the view of the game where the panel with the 
	 * game status is at the top, and a panel to the display of the game is in 
	 * the middle.  The two panels get the needed game information via the 
	 * GameInfoProvider, and the game panel passes key events to the keyListener.
	 ...
	 */
	public void showNewGameView(GameInfoProvider gameInfoProvider,
